...
import { getAllPodcasts, getRecentEpisodes } from "@/lib/podcasts-server";
...
export default async function PodcastsIndexPage() {
  const [shows, recent] = await Promise.all([getAllPodcasts(), getRecentEpisodes(20)]);
        ...
        {shows.length === 0 ? (
          ...
          <>
            ...
            {recent.length > 0 && (
              <section>
                <h2 className="font-display font-black text-lg uppercase tracking-tight text-text-primary mb-4">
                  Recent <span className="text-brand-red">Episodes</span>
                </h2>
