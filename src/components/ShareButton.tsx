...
export default function ShareButton({ text, url, hashtags = "BKGrit,Nets,NBADraft", size = "sm" }: ShareButtonProps) {
  const href = tweetUrl(text, url || "https://bkgrit.com?v=2", hashtags);
...
export function ShareLotteryResult({ pick }: { pick: number }) {
  ...
  const href = tweetUrl(text, `https://bkgrit.com/share/lottery?pick=${pick}`, "BKGrit,Nets,NBADraft");
...
export function ShareGMResult({
  ...
  const href = tweetUrl(text, `https://bkgrit.com/share/gm?score=${score}&grade=${encodeURIComponent(grade)}&player=${encodeURIComponent(player)}&percentile=${percentile}`, "BKGrit,Nets,NBADraft");
